...
long long parse(string &sentence, int index, IWCMAP &iwc, long long total[]) { 
	...
	if(total[index] == -1) {
		...
		for(auto &kv : iwc) {
			...
			for(int i = length; i < n; ++ i) {
				word.addc(sentence[index + i]);
			}
			length = n;
			
			auto it = wc.find(word);
			if(it != wc.end()) {
				total[index] += it->second * parse(sentence, index + n, iwc, total) % MOD;
			}
...
int main() {
	...
	for(int i = 1; i <= T; ++ i) {
		...
		for(int j = 0; j < S; ++ j) { 
			...
			memset(total, -1, sentence.length());

			s.parse(sentence, 0, 1, iwc, total);
			cout << total[0]; 
		}
